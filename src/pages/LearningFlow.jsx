...
const LearningFlow = () => {
    ...
    const handleStartBatchTest = () => {
        ...
        // TODO: TEMPORARY - Limit to 10 questions for testing
        // PRODUCTION: Should test all words in batch (30 total)
        const limitedTestWords = testWords.slice(0, 10);

        setBatchTestWords(limitedTestWords.map(w => w.id));
        setBatchTestIndex(0);
        setBatchTestResults([]);
        setCurrentMode('batch_test');

        // Generate first question
        generateBatchTestQuestion(limitedTestWords[0].id);
    };
    ...
    const showBatchTestResults = () => {
        ...
        // TODO: TEMPORARY - Require only 5/10 correct (50%) for testing
        // PRODUCTION: Should require 27/30 correct (90%) to pass
                ...
                {currentMode === 'batch_test_results' && (
                    <QuizFeedback
                        ...
                        quizSummary={{
                            ...
                            weak_words: batchTestResults.filter(r => !r.correct).map(r => words.find(w => w.id === r.wordId)?.word || `Word #${r.wordId}`),
                            time_taken: null
                        }}
                        ...
                        <WordCard
                            ...
                            onNext={() => {
                                if (reviewSessionIndex + 1 < reviewSessionWords.length) {
                                    ...
                                    alert('Review session complete! Great work! 🎉');
                                    setReviewSessionWords([]);
                                    setReviewSessionIndex(0);
                                    setCurrentMode('batch_complete');
                                }
