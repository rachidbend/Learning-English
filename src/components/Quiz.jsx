...
const Quiz = ({
    ...
    const renderExplanation = () => {
        const { question_type, metadata, explanation } = question;

        if (explanation) {
            return (
                <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Explanation:</p>
                    <p className="text-sm text-gray-800 italic">{explanation}</p>
                </div>
            );
        }
    ...
    return (
        <div className="w-full bg-white rounded-2xl shadow-lg p-6 min-h-[500px]">
            ...
            {/* Feedback Section */}
            {!isQuickMode && showFeedback && !isCorrect && (
                <div className="rounded-2xl p-6 mb-6 bg-error/10 border-2 border-error">
